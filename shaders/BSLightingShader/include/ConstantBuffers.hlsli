...
cbuffer PerTechniqueCB : register(b0)
{
    float4 HighDetailRange                       : packoffset(c0);      // @ 0 - 0x0000
    float4 FogParam                              : packoffset(c1);      // @ 4 - 0x0010
    float4 FogNearColour                         : packoffset(c2);      // @ 8 - 0x0020
    float4 FogFarColour                          : packoffset(c3);      // @ 12 - 0x0030
}
...
cbuffer PerGeometryCB : register(b2)
{
...
#if defined(SKINNED)
cbuffer PreviousBonesCB : register(b9)
{
    float4 PreviousBones[240];
}
cbuffer BonesCB : register(b10)
{
    float4 Bones[240];
}
#endif
...
cbuffer PerTechniqueCB : register(b0)
{
    float4 FogColour                             : packoffset(c0);      // @ 0 - 0x0000
...
cbuffer PerGeometryCB : register(b2)
{
    ...
    float3 DirLightColour                        : packoffset(c1);      // @ 4 - 0x0010
    ...
    float3 EmitColour                            : packoffset(c4.y);    // @ 17 - 0x0044
    ...
    float4 PointLightColour[7]                   : packoffset(c22);     // @ 88 - 0x0160
...
cbuffer PerFrameCB : register(b12)
{
